...
OUT_PATH = Path("events_TS_structured_roberta.jsonl")

# Optional: external domain lexicon (you can leave this file empty or not create it yet)
LEXICON_PATH = Path("data_aggregation/domain_lexicon.json")

# Toggle acronym heuristic
INCLUDE_ACRONYMS = True

# ---------- Load distilled RoBERTa NER ----------
...
ner_pipe = pipeline(
    ...
    model=model,
    tokenizer=tokenizer,
    aggregation_strategy="simple",  # merges subwords into complete tokens
)
...
# ---------- Entity extraction with distilRoBERTa NER ----------
...
def extract_entities(text: str) -> List[str]:
    """
    E = entities from:
      - distilRoBERTa NER (primary)
      - optional domain lexicon
      - optional ALLCAPS acronym heuristic
    """
    if not text:
        return []
    ...
    # 1) HuggingFace NER
    outputs = ner_pipe(text)
    for ent in outputs:
        # ent["word"] is the stitched token span
        word = normalize_ent_text(ent["word"])
        if word:
            ents.add(word)

    # 2) Domain lexicon (macro, crypto, sports, etc. – you expand this JSON over time)
    ...
    # 3) ALLCAPS acronyms (optional)
    if INCLUDE_ACRONYMS:
        for token in re.findall(r"\b[A-Z][A-Z0-9]{1,}\b", text):
            # filter out obviously non-signal tokens if needed
            if len(token) >= 3:
                ents.add(normalize_ent_text(token))
...
def infer_resolution_type(obj: Dict[str, Any]) -> str:
    """
    psi: 'binary', 'scalar', 'categorical', 'combo'
    """
